...
export async function updateMember(prevState, formData) {
  ...
  if (errors.length > 0) {
    return {
      errors,
      values: { id, member_code, member_name, member_major },
    };
  }
 
  try {
    await db.query(
      "UPDATE member SET member_code=?, member_name=?, member_major=? WHERE id=?",
      [member_code, member_name, member_major, id]
    );
  } catch (err) {
  ...
  redirect("/admin/members?success=update");
}
